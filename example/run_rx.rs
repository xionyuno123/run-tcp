...
fn main() {
    ...
    while run.load(Ordering::Acquire) {
        ...
        println!(
            "total rx: {} pps, {} Gbps, {} misses/s",
            (curr_stats.ipackets() - old_stats.ipackets()) / sleep_second,
            ((curr_stats.ibytes() - old_stats.ibytes()) as f64 * 8.0 / 1000000000.0) / sleep_second as f64,
        ...
        for qid in 0..nb_qs as usize {
            print!(
                ...
                (bps_stats[qid].load(Ordering::SeqCst) as f64 * 8.0 / 1000000000.0) / sleep_second as f64,
            );
        }
        println!("");
